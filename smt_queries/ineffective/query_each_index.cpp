...
//   }
  ...
  // 1. Check if the baseline setup itself is SAT without forcing any output divergence
  slv.push();
  ...
  if (base_res != sat) {
    cout << "[!] CRITICAL ERROR: The base 4-trace framework is UNSAT or TIMEOUT before adding any output divergence!\n";
    cout << "    This means input setup, trace concatenation, or pointer pinning creates a contradiction.\n";
    slv.pop();
    return 1;
  }
  slv.pop();
...
//   // =====================================================================
//   // TESTING OUTPUT INDICES INDIVIDUALLY
//   // =====================================================================
//   cout << "Testing output indices individually for divergence...\n";

//   for (long long diff_idx = 0; diff_idx < OUTPUT_REGION.length; diff_idx++) {
//     slv.push();

//     expr diff_addr = ctx.int_val((int)(OUTPUT_REGION.offset + diff_idx)) + sPtr;
//     expr c2_val = select(finC2, diff_addr);
//     expr f2_val = select(finF2, diff_addr);

//     // Force divergence at index `diff_idx`
//     slv.add(c2_val != f2_val);

//     cout << "\nChecking Index [" << diff_idx << "] with expression: " << (c2_val != f2_val) << " ... " << flush;
...
//       cout << "SAT!\n";
//       cout << "\n======================================================\n";
//       cout << "SAT — found divergence target at output index [" << diff_idx << "]\n";
//       cout << "======================================================\n\n";
...
//       cout << "\n-- Output region, execution 2 (observable target index: " << diff_idx << ") --\n";
//       for (long long i = 0; i < OUTPUT_REGION.length; i++) {
//         expr addr = ctx.int_val((int)(OUTPUT_REGION.offset + i)) + sPtr;
